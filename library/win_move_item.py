...
DOCUMENTATION = '''
---
module: win_copy_item_item
#version_added: "1.9.2"
short_description: Copies files remote locations on windows hosts.
description:
     - The M(win_copy_item) module copies a file on the local box to remote windows locations.
options:
  src:
    description:
      - Remote path to a file to copy to the remote server; can be absolute or relative.
...
notes:
   - The "win_copy_item" module is best used for small files only.
...
EXAMPLES = '''
# Copy a single file
- win_copy_item: src=/srv/myfiles/foo.conf dest=c:\\TEMP\\foo.conf
...
- win_copy_item: src=files/temp_files/ dest=c:/temp/
